...
class RecipeCalculator:
    ...
    def select_best_machine(self, recipe_category: str) -> str:
        ...
        if best_machine is None:
            fallback = 'assembling_machine' if 'assembling_machine' in machines else list(machines.keys())[0]
            logger.debug(f"No machine found for category '{recipe_category}', using fallback: {fallback}")
            return fallback
